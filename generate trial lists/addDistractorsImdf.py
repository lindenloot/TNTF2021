"""
TODO
Merge two addDistractor modules together
"""
...
def combine(dm, l_neg_dist1,l_neg_dist2 ,l_neu_dist1 ,l_neu_dist2):
    ...
    for row in dm:
        ...
        # Randomly select two possible distractors (from the same category as the target object)
        ...
        trial_dm["distractor_object_1"] = dist1
        trial_dm["distractor_object_2"] = dist2
...
def addDistractors(dm):
    ...
    # Make a list of potential objects for negative distractor 1
    L_NEG_DIST1 = list(neg_dm.Object)
    # and negative distractor 2
    L_NEG_DIST2 = list(neg_dm.Object)
    
    # Make a list of potential objects for neutral distractor 1
    L_NEU_DIST1 = list(neu_dm.Object)
    # and 2
    L_NEU_DIST2 = list(neu_dm.Object)

    new_dm = combine(dm, L_NEG_DIST1[:], L_NEG_DIST2[:], L_NEU_DIST1[:], L_NEU_DIST2[:])

    # Make sure target, distractor 1 and distractor 2 are different objects:
    while any(
        row.distractor_object_1 == row.distractor_object_2 or
        row.distractor_object_1 == row.Object or
        row.distractor_object_2 == row.Object
        for row in new_dm
        ):
        # If not, try again:
         print("try again")
